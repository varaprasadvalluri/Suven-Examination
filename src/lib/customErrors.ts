...
export class CustomAppException extends Error {
  ...
  constructor(action: string, friendlyMessage: string, technicalDetails: string, code: string = 'UNKNOWN_EXCEPTION') {
...
export function mapToCustomException(error: any, actionContext: string): CustomAppException {
  ...
  const technicalDetails = rawMessage;

  // 1. Firebase Authentication Errors
  if (rawCode === 'auth/email-already-in-use' || rawMessage.includes('email-already-in-use')) {
    code = 'auth/email-already-in-use';
    friendlyMessage =
      'This institutional email address is already registered. If you already created an account, please head to the Sign In tab instead of Sign Up.';
  } else if (rawCode === 'auth/invalid-email' || rawMessage.includes('invalid-email')) {
    code = 'auth/invalid-email';
    friendlyMessage =
      'The email address you entered has an invalid format. Please double-check for spelling mistakes, trailing spaces, or missing characters.';
  } else if (rawCode === 'auth/weak-password' || rawMessage.includes('weak-password')) {
    code = 'auth/weak-password';
    friendlyMessage =
      'The password provided is too simple or weak. To safeguard your account, please enter a stronger password containing at least 6 characters.';
  } else if (
    ...
    friendlyMessage =
      'Invalid credentials. The email address or password you entered is incorrect, or no profile matches these credentials under your chosen role.';
  } else if (rawCode === 'auth/too-many-requests' || rawMessage.includes('too-many-requests')) {
    code = 'auth/too-many-requests';
    friendlyMessage =
      'Too many login attempts detected. To protect your profile, access has been temporarily restricted. Please wait a minute and try again.';
  } else if (rawCode === 'auth/operation-not-allowed' || rawMessage.includes('operation-not-allowed')) {
    code = 'auth/operation-not-allowed';
    friendlyMessage =
      'The requested login method is currently disabled for this portal. Please contact the main administrator for support.';
  } else if (rawCode === 'auth/popup-closed-by-user' || rawMessage.includes('popup-closed-by-user')) {
    code = 'auth/popup-closed-by-user';
    friendlyMessage =
      'The authentication pop-up was closed before completion. Please try clicking the button again and leave the window open to finish.';
  }
  ...
  // 2. Firebase Firestore Database & Security Errors
    ...
    friendlyMessage =
      'Access Denied: You do not have the required institutional clearance or security role permissions to read or update this resource.';
  } else if (rawCode === 'unavailable' || rawMessage.includes('unavailable')) {
    code = 'unavailable';
    friendlyMessage =
      'The secure data server is temporarily offline or unreachable. Please check your network connection and retry the action.';
  } else if (rawCode === 'already-exists' || rawMessage.includes('already-exists')) {
    code = 'already-exists';
    friendlyMessage =
      'A duplicate entry was found. This database record already exists and cannot be created again with identical primary details.';
  } else if (rawCode === 'not-found' || rawMessage.includes('not-found')) {
    code = 'not-found';
    friendlyMessage =
      'The requested database record, institutional workspace, or exam could not be located. It may have been deleted or moved.';
  } else if (rawCode === 'resource-exhausted' || rawMessage.includes('resource-exhausted')) {
    code = 'resource-exhausted';
    friendlyMessage =
      'Server capacity quota reached. The application is receiving high traffic; please wait a brief moment before resubmitting.';
  }
  ...
  // 3. Custom Whitelist & Onboarding Exceptions
    ...
    friendlyMessage =
      'Your institutional email has not been onboarded or pre-registered by the System Administrator yet. Please request onboarding from the master administrator before registering.';
  ...
  if (code === 'UNKNOWN' && rawMessage) {
    if (rawMessage.includes('Failed to fetch') || rawMessage.includes('NetworkError')) {
      ...
      friendlyMessage =
        'A secure network connection could not be established. Please check your local Wi-Fi or cellular network connection and try again.';
    }
  }

  return new CustomAppException(actionContext, friendlyMessage, technicalDetails, code);
}
...
export function handleErrorAndLog(error: any, actionContext: string): CustomAppException {
  ...
  const isBackgroundOrNonFatal =
    actionContext.includes('Sync') ||
  ...
  const handleUnhandledRejection = (event: PromiseRejectionEvent) => {
    ...
    if (reason?.stack) {
      ...
      if (stackStr.includes('signIn') || stackStr.includes('login') || stackStr.includes('Auth')) {
        ...
        stackStr.includes('addDoc') ||
        stackStr.includes('setDoc') ||
        stackStr.includes('updateDoc') ||
        stackStr.includes('schools')
      ) {
