...
export async function deliverIdempotentNotification(notificationId: string): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    const notificationRecord = await runTransaction(db, async (transaction) => {
    ...
    console.log(
      `[Idempotent Dispatch] Acquired lock for ${notificationId}. Triggering output channel API: ${notificationRecord.channel}...`
    );
    ...
    try {
      if (notificationRecord.channel === 'sms') {
        dispatchSuccess = await simulateTwilioSmsDispatch(notificationRecord.destination, notificationRecord.linkUrl);
      } else {
        dispatchSuccess = await simulateSendGridEmailDispatch(notificationRecord.destination, notificationRecord.linkUrl);
      }
    } catch (apiError: any) {
...
export async function executeBackgroundBulkPdfGeneration(
  ...
  try {
    ...
    for (let i = 0; i < studentAttempts.length; i++) {
      ...
      console.log(
        `[Cloud Function pdfGen] Compiling Vector PDF Report Card for student ${student.studentName} (${i + 1}/${studentAttempts.length})`
      );
