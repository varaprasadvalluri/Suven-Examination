...
export const AdminExams: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const secureLinksQuery = query(collection(db, 'secure_exam_links'), where('schoolId', '==', profile.schoolId));
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.docs.forEach((doc) => {
          const linkRecord = doc.data();
          linksMap[linkRecord.examId] = linkRecord.id; // examId -> secure token id value
        });
  ...
  const filteredExams = useMemo(() => {
    let list = exams.filter(
      (e) => e.title.toLowerCase().includes(searchQuery.toLowerCase()) || e.subject.toLowerCase().includes(searchQuery.toLowerCase())
    );
          ...
          const permittedExams =
            profile.role === 'admin'
              ...
              : fetchedExams.filter((e) => {
                  ...
                  const isAssigned =
                    !e.assignedSchoolIds || e.assignedSchoolIds.length === 0 || e.assignedSchoolIds.includes(profile.schoolId);
  ...
  const handleToggleStatus = async (exam: Exam) => {
    ...
    try {
      ...
      if (nextStatus === 'published') {
        let schoolsToProvision = exam.assignedSchoolIds && exam.assignedSchoolIds.length > 0 ? exam.assignedSchoolIds : [];
  ...
  return (
    <DataLoader isLoading={loading} error={error} onRetry={handleRetry} loadingMessage="Loading Exam Registers...">
      <div className="space-y-8 animate-in fade-in duration-500">
          ...
          {(
            ...
            <button
              ...
              className={`px-5 py-2 text-xs font-bold rounded-lg transition-all ${
                activeTab === tab.key ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-500 hover:text-slate-950 hover:bg-slate-50'
              }`}
            >
              {tab.label}
            </button>
            ...
            {filteredExams.slice((page - 1) * pageSize, page * pageSize).map((exam) => {
              ...
              if (exam.startTime) {
                ...
                if (!isNaN(startDate.getTime())) {
                  dateStr = startDate.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' });
              ...
              return (
                <Card
                  key={exam.id}
                  className="relative overflow-hidden group border border-slate-100 bg-white rounded-[20px] p-6 shadow-sm hover:shadow-md transition-all flex flex-col justify-between min-h-[220px]"
                >
                  {/* Top Row: Code & Badge */}
                  ...
                  <div className="flex-grow">
                    <div className="flex items-start justify-between gap-4 mt-2">
                      <div>
                        ...
                        <p className="text-xs font-semibold text-slate-400 mt-1">Class XII · {exam.totalMarks > 100 ? 72 : 68} students</p>
                      </div>
            ...
            {exams.length === 0 && (
              <div className="col-span-full py-32 text-center border-2 border-dashed border-slate-200 rounded-2xl bg-white shadow-inner animate-in fade-in">
                ...
                <p className="text-slate-500 mb-8 max-w-sm mx-auto">
                  Welcome to the System Administration portal. Start by creating an examination or managing institutional branches.
                </p>
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <Button className="bg-indigo-600 hover:bg-indigo-700 shadow-lg px-8" onClick={() => setIsCreateOpen(true)}>
                    Create First Exam
                  </Button>
                  {profile?.role === 'admin' && (
                    <Button
                      variant="outline"
                      className="border-slate-200 bg-white shadow-sm px-8"
            ...
            <div className="p-6 border border-slate-200 rounded-[24px] flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50/50">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-slate-400 ml-4">
                  Showing {Math.min(filteredExams.length, (page - 1) * pageSize + 1)} - {Math.min(filteredExams.length, page * pageSize)} of{' '}
        ...
        {selectedDispatchExam && (
          <AdminDispatchCenter exam={selectedDispatchExam} isOpen={!!selectedDispatchExam} onClose={() => setSelectedDispatchExam(null)} />
        )}
        ...
        {isEditOpen && editingExam && (
          ...
          >
            <DialogContent className="sm:max-w-[750px] p-0 overflow-hidden rounded-[24px] border border-slate-100 shadow-2xl bg-white">
              <div className="bg-gradient-to-r from-slate-50 via-indigo-50/10 to-slate-50 px-5 md:px-8 py-6 border-b border-slate-100 flex items-center justify-between gap-3">
                <div>
                  <DialogTitle className="font-display font-extrabold text-xl tracking-tight text-slate-900">
                    Edit Assessment Criteria
                  </DialogTitle>
                  <DialogDescription className="text-slate-500 text-xs mt-1">
                    Reset timelines, modify parameters, or adjust institutional allocation.
                  </DialogDescription>
                </div>
              ...
              <div className="p-5 md:p-8 max-h-[65vh] overflow-y-auto space-y-6 bg-white">
                <div className="space-y-2">
                  <Label htmlFor="edit-title" className="text-xs font-black uppercase tracking-widest text-slate-500">
                    Academic Title
                  </Label>
                  <Input
                    id="edit-title"
                ...
                <div className="space-y-2">
                  <Label htmlFor="edit-desc" className="text-xs font-black uppercase tracking-widest text-slate-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select value={editExamState.subject} onValueChange={(val) => setEditExamState((prev) => ({ ...prev, subject: val }))}>
                      <SelectTrigger
                        id="edit-subject"
                        className="h-12 bg-slate-50/30 hover:bg-white rounded-xl border-slate-200 text-sm transition-all"
                      >
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="edit-difficulty" className="text-xs font-black uppercase tracking-widest text-slate-500">
                      Target Difficulty
                    </Label>
                    <Select
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="edit-duration" className="text-xs font-black uppercase tracking-widest text-slate-500">
                      Duration (Minutes)
                    </Label>
                    <Input
                      id="edit-duration"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="edit-marks" className="text-xs font-black uppercase tracking-widest text-slate-500">
                      Total Marks
                    </Label>
                    <Input
                      id="edit-marks"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-slate-100 pt-4">
                  <div className="space-y-2">
                    <Label htmlFor="edit-start" className="text-xs font-black uppercase tracking-widest text-slate-500">
                      Start Window (Reset)
                    </Label>
                    <Input
                      id="edit-start"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="edit-end" className="text-xs font-black uppercase tracking-widest text-slate-500">
                      End Window (Reset)
                    </Label>
                    <Input
                      id="edit-end"
            ...
            <DialogContent className="sm:max-w-[700px] max-h-[90vh] flex flex-col p-0 overflow-hidden bg-white border-0 shadow-2xl rounded-2xl">
              ...
              <DialogFooter className="p-6 bg-slate-50 border-t border-slate-100">
                <Button
                  ...
                  className="w-full h-11 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold text-xs transition-colors"
                >
                  Close Preview
                </Button>
              </DialogFooter>
