...
import { Calendar, CheckCircle2, Circle, Clock, Map, PlayCircle, BookOpen, BrainCircuit, ChevronRight, Star, Zap } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-11 items-start">
        ...
        <div className="lg:col-span-4 space-y-4 w-full min-w-0">
          {ROADMAP_DATA.map((day, idx) => (
            <button
              ...
              className={`w-full text-left p-5 md:p-6.5 rounded-[24px] border transition-all duration-300 relative overflow-hidden group ${
                activeDay === idx
            ...
            >
              ...
              <div className="flex items-center gap-4">
                <div
                  className={`h-12 w-12 rounded-xl flex flex-col items-center justify-center font-display transition-all shrink-0 ${
                ...
                <div className="min-w-0 flex-1">
                  <h3
                    className={`font-black text-base leading-tight transition-colors ${activeDay === idx ? 'text-white' : 'text-slate-400'}`}
                  >
                    {day.date}
                  </h3>
                  <p
                    className={`text-[10px] font-black uppercase tracking-widest mt-1.5 truncate ${activeDay === idx ? 'text-indigo-400' : 'text-slate-500'}`}
                  >
                    {day.milestone}
                  </p>
                </div>
        ...
        <div className="lg:col-span-8 w-full">
          <Card className="bg-gradient-to-br from-[#13162C] to-[#121326] border border-[#20254C] rounded-[32px] overflow-hidden shadow-2xl min-h-[500px] flex flex-col w-full">
            <CardHeader className="bg-[#121429]/95 p-5 md:p-8 border-b border-[#1E2342]">
              <div className="flex items-center justify-between flex-wrap gap-4">
                <div>
                  <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1.5 leading-none">
                    {ROADMAP_DATA[activeDay].milestone}
                  </p>
                  <CardTitle className="text-2xl font-black text-white tracking-tight uppercase leading-none font-display">
                    {ROADMAP_DATA[activeDay].date}
                  </CardTitle>
                </div>
            ...
            <CardContent className="p-5 md:p-8 flex-1 space-y-6">
                    ...
                    <div
                      className={`h-11 w-11 rounded-xl flex items-center justify-center shrink-0 transition-all ${
                        topic.type === 'study'
                          ...
                          : 'bg-orange-500/10 text-orange-300 border border-orange-500/20 shadow-sm'
                      }`}
                    >
                      {topic.type === 'study' ? <BookOpen className="h-6 w-6" /> : <BrainCircuit className="h-6 w-6" />}{' '}
                      {/* Locked icons 24x24 px */}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap mb-1.5">
                        <Badge className="bg-indigo-500/10 border-none text-indigo-300 text-[9px] font-black uppercase tracking-wider px-2 py-0.5">
                          {topic.type}
                        </Badge>
                        <span className="text-[10px] font-mono text-slate-450 font-bold">{topic.time} ALLOCATED</span>
                      </div>
                      <h4 className="text-lg font-black text-white group-hover:text-indigo-400 transition-colors uppercase tracking-tight truncate">
                        {topic.title}
                      </h4>
                    </div>
                  </div>
              ...
              <div className="mt-8 p-5 md:p-6.5 bg-[#121429]/40 rounded-[24px] border border-[#1E2342] flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-base font-black text-slate-100 mt-2 uppercase tracking-tight leading-none">
                      Master 3 Complex Physics Heuristics 🌟
                    </p>
                  </div>
                </div>
                ...
                <div className="flex -space-x-3 select-none">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="h-9 w-9 rounded-full border-2 border-[#1E2342] bg-[#14172B] overflow-hidden shrink-0">
                      <img
                        src={`https://api.dicebear.com/7.x/avataaars/svg?seed=student-${i + 2}`}
                        alt="Peer avatar"
                        referrerPolicy="no-referrer"
                      />
                    </div>
                  ))}
                  <div className="h-9 w-9 rounded-full border-2 border-[#1E2342] bg-indigo-600 flex items-center justify-center text-[9px] font-black text-white shrink-0">
                    +42
                  </div>
                </div>
              </div>
