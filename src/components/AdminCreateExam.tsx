    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-12 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-[28px] border border-slate-200/80 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-2xl font-display font-black text-slate-900 tracking-tight mt-0.5">Create Assessment Portal</h1>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[32px] border border-slate-200/80 shadow-xl overflow-hidden">
        <div className="flex flex-col md:flex-row min-h-[580px]">
          {/* Stepper Sidebar */}
          <div className="md:w-72 bg-slate-900 p-6 md:p-8 text-white flex flex-col justify-between shrink-0">
            <div>
              ...
              <div className="space-y-6">
                {[
                  ...
                  >
                    <div
                      className={`h-9 w-9 rounded-xl border-2 flex items-center justify-center text-xs font-black transition-all ${step >= s.id ? 'bg-indigo-600 border-indigo-600 text-white shadow-md shadow-indigo-600/20' : 'border-slate-800 text-slate-500 bg-slate-950/40'}`}
                    >
                      {step > s.id ? <CheckCircle2 size={16} /> : s.id}
                    </div>
                    <div>
                      <p
                        className={`text-xs font-black uppercase tracking-wider leading-none ${step === s.id ? 'text-white' : step > s.id ? 'text-slate-300' : 'text-slate-500'}`}
                      >
                        {s.label}
                      </p>
          ...
          <div className="flex-grow p-6 sm:p-8 md:p-12 bg-white flex flex-col justify-between">
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs font-bold text-slate-400 mt-1">
                        Establish the identity and metadata of this assessment portal.
                      </p>
                    </div>
                ...
                {step === 2 && (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs font-bold text-slate-400 mt-1">
                        Set operational metrics, duration, and score weighting for this exam.
                      </p>
                    </div>
                ...
                {step === 3 && (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs font-bold text-slate-400 mt-1">
                        Define the valid temporal window and institutional targeting for student access.
                      </p>
                    </div>
                      ...
                      <div className="pt-4 border-t border-slate-100 space-y-4">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                          <Button
                            ...
                            className={`text-xs font-bold uppercase tracking-wider rounded-xl h-10 px-4 cursor-pointer w-full sm:w-auto ${newExamMode === 'global' ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-transparent text-slate-700 border-slate-200'}`}
                          >
                          ...
                          <Button
                            ...
                            className={`text-xs font-bold uppercase tracking-wider rounded-xl h-10 px-4 cursor-pointer w-full sm:w-auto ${newExamMode === 'specific' ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-transparent text-slate-700 border-slate-200'}`}
                          >
                            ...
                            {schools.length === 0 ? (
                              ...
                              schools.map((school) => {
                                ...
                                return (
                                  <label
                                    key={school.id}
                                    className="flex items-center gap-3 cursor-pointer p-1.5 hover:bg-slate-100/80 rounded-lg transition-colors"
                                  >
                                    <input
                                      type="checkbox"
                                      checked={checked}
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-bold text-slate-400">
                          Final verification required before deploying to the national examination database.
                        </p>
                      </div>
                    </div>
                        ...
                        <span className="text-xs font-bold text-slate-900 bg-white p-2.5 rounded-xl border border-slate-200 block">
                          {newExamMode === 'global' ? (
                            ...
                            <span className="text-indigo-600 font-bold">
                              Assigned Schools:{' '}
                              {newExam.assignedSchoolIds.map((id) => schools.find((s) => s.id === id)?.name || id).join(', ') ||
                                'None selected'}
                            </span>
                          )}
                        </span>
            ...
            <div className="mt-8 pt-6 border-t border-slate-100 flex items-center justify-between gap-4">
              ...
              {step < 4 ? (
                <Button
                  type="button"
                  onClick={() => {
                    if (canAdvanceStep(step)) setStep(step + 1);
                  }}
