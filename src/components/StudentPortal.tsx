...
const MathIcon: React.FC = () => (
  <svg
    className="h-6 w-6 shrink-0 transform group-hover:scale-110 group-hover:rotate-3 transition-transform"
    viewBox="0 0 100 100"
    fill="none"
  >
...
const ScienceIcon: React.FC = () => (
  ...
  >
    ...
    <path
      d="M 38 15 H 62 M 45 15 V 40 L 22 76 C 19 81 23 88 30 88 H 70 C 77 88 81 81 78 76 L 55 40 V 15"
      stroke="black"
      strokeWidth="3.5"
      strokeLinecap="round"
...
const LanguageIcon: React.FC = () => (
  ...
  >
    <path
      d="M 22 18 H 70 C 76 18 78 22 78 28 V 78 C 78 84 74 86 68 86 H 22 Z"
      fill="#FFB2B8"
      stroke="black"
      strokeWidth="3.5"
      strokeLinejoin="round"
...
export const StudentPortal: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const applyMergedExams = () => {
      ...
      const permittedExams = studentSchoolId
        ? fetchedExams.filter(
            (e) => !e.assignedSchoolIds || e.assignedSchoolIds.length === 0 || e.assignedSchoolIds.includes(studentSchoolId)
          )
        : fetchedExams;
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in duration-500 max-w-md mx-auto">
        ...
        <p className="text-slate-600 mt-3 font-semibold text-sm leading-relaxed">
          Your current security clearance does not possess the required <code>take_exams</code> authority token. Reach out to your academy
          registrar.
        </p>
  ...
  const filteredExams = exams.filter((exam) => {
    ...
    const matchesSearch =
      exam.title.toLowerCase().includes(searchQuery.toLowerCase()) || subjectStr.toLowerCase().includes(searchQuery.toLowerCase());
      ...
      <Tabs value={activeMainTab} onValueChange={setActiveMainTab} className="space-y-11">
        ...
        {profile?.role !== 'student' && (
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 bg-white p-6 md:p-8 rounded-[32px] border-2 border-b-[8px] border-slate-200 shadow-[4px_6px_0px_rgba(163,191,220,0.15)]">
            <div>
              <h1 className="text-2xl md:text-3xl font-black text-indigo-950 font-display uppercase tracking-tight">
                🪐 Student Portal Cosmos
              </h1>
              <p className="text-slate-500 font-bold text-xs mt-1 md:mt-2">
                Embark on daily educational quests, master diagnostic quizzes, and track milestones!
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-6 px-2 font-display">
              <div className="flex flex-col items-end">
                ...
                <span className="text-xs font-black text-emerald-600 flex items-center gap-1.5 mt-1.5 bg-emerald-50 border-2 border-emerald-200 px-3 py-1 rounded-full">
                  +12.4% <ArrowUpRight className="h-4 w-4 shrink-0" strokeWidth={3} />
                </span>
              </div>
              ...
              <div className="flex flex-col items-end">
                ...
                <span className="text-xs font-black text-amber-700 mt-1.5 bg-amber-50 border-2 border-amber-200 px-3 py-1 rounded-full">
                  #42 / 2,410
                </span>
              </div>
            </div>
        ...
        <TabsContent value="exams" className="space-y-11 outline-none">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-11 items-start">
            ...
            <div className="lg:col-span-12 space-y-11">
              ...
              <div className="bg-[#EBF3FF] rounded-[32px] p-5 md:p-8 border-2 border-b-[8px] border-indigo-200 shadow-sm relative overflow-hidden min-w-[280px] sm:min-w-[320px]">
                ...
                <div className="relative z-10 space-y-4">
                  ...
                  <p className="text-slate-600 text-sm md:text-base font-semibold max-w-xl leading-relaxed">
                    Assemble your logic gears and unlock daily milestones! Take e-exams to gather magical stardust, study gems, and academy
                    trophies. Ready to showcase your wizard powers? 🚀✨
                  </p>
              ...
              {(() => {
                ...
                return (
                  <div className="bg-white border-2 border-b-[8px] border-slate-800 rounded-[32px] p-5 md:p-8 shadow-[4px_6px_0px_rgba(0,0,0,0.15)] relative overflow-hidden space-y-6">
                    ...
                    <div className="space-y-2 max-w-md md:max-w-xl">
                      <span className="text-[10px] font-black text-rose-500 uppercase tracking-widest bg-rose-50 border border-rose-200 px-3 py-1 rounded-full">
                        ACTIVE STAR MISSION
                      </span>
                      <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight mt-2">{activeQuestExam.title}</h3>
                      <p className="text-slate-500 text-xs font-semibold leading-relaxed">
                        Your active brain power course is currently ready for launch. Complete the checklist checkpoints to fetch maximum
                        points today!
                      </p>
                    </div>
                        ...
                        {[
                          ...
                          return (
                            <div key={index} className="flex flex-col items-center relative z-10 text-center">
                              <div
                                className={`w-11 h-11 rounded-full flex items-center justify-center border-2 border-b-[4px] ${
                                  isDone
                                    ? 'bg-emerald-400 border-slate-800'
                                    : isActive
                                      ? 'bg-amber-400 border-slate-800 animate-pulse'
                              ...
                              <span
                                className={`text-[10px] uppercase font-black tracking-wider mt-2.5 ${
                                  isDone ? 'text-emerald-600' : isActive ? 'text-amber-600 font-extrabold' : 'text-slate-400'
                                }`}
                              >
                                {node.name}
                              </span>
                ...
                {[
                  {
                    ...
                    sub: 'Knowledge tags',
                    icon: CheckCircle2,
                    accBg: 'bg-emerald-100 text-emerald-800 border-emerald-300',
                    dot: 'bg-emerald-500'
                  },
                  {
                    label: 'Cognitive Effort',
                    value: attempts.length > 0 ? 'Optimal' : 'Active',
                    sub: 'Careful & Steady',
                    icon: TrendingUp,
                    accBg: 'bg-amber-100 text-amber-800 border-amber-300',
                    dot: 'bg-amber-505'
                  },
                  {
                    ...
                    sub: 'Open pathways',
                    icon: BookOpen,
                    accBg: 'bg-sky-100 text-sky-850 border-sky-300',
                    dot: 'bg-sky-500'
                  },
                  {
                    label: 'Safety Seal',
                    value: 'Lv. 1 Safe',
                    sub: 'Proctor verified',
                    icon: ShieldCheck,
                    accBg: 'bg-rose-100 text-rose-800 border-rose-300',
                    dot: 'bg-rose-500'
                  }
                ].map((stat, i) => (
                  <div
                    key={i}
                    className="p-6 rounded-[24px] bg-white border-2 border-b-[6px] border-slate-200 shadow-sm transition-all hover:translate-y-[-2px] hover:shadow-md flex flex-col justify-between min-w-[195px]"
                  >
                    <div className="flex items-center justify-between gap-3 mb-4">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-widest text-[#64748B] text-slate-450 truncate">
                        {stat.label}
                      </span>
                    </div>
                    <div>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-6 bg-white border-2 border-b-[6px] border-slate-200 rounded-[28px] shadow-sm">
                <div>
                  <h3 className="text-xl font-extrabold text-indigo-950 tracking-tight leading-none uppercase font-display">
                    Quiz Universe Catalogue
                  </h3>
              ...
              {filteredExams.length === 0 ? (
                <div className="p-16 text-center bg-white border-2 border-b-[6px] border-slate-200 rounded-[32px] max-w-xl mx-auto space-y-4 shadow-sm">
                  ...
                  <p className="text-xs text-slate-450 font-semibold leading-relaxed">
                    Try searching for other key terms to launch active exam simulations.
                  </p>
                </div>
              ) : (
                  ...
                  {filteredExams.map((exam) => {
                    ...
                    const borderTheme =
                      exam.subject === 'Mathematics'
                        ? 'border-amber-300 hover:border-amber-400 border-b-[8px]'
                        : exam.subject === 'Physics'
                          ? 'border-emerald-300 hover:border-emerald-400 border-b-[8px]'
                          : 'border-sky-305 hover:border-sky-400 border-b-[8px]';
                    const badgeTheme =
                      exam.subject === 'Mathematics'
                        ? 'text-amber-800 bg-amber-50 border-amber-200'
                        : exam.subject === 'Physics'
                          ? 'text-emerald-800 bg-emerald-50 border-emerald-202'
                          : 'text-sky-850 bg-sky-50 border-sky-200';
                        ...
                        <div className="relative p-7 md:p-8 space-y-5 flex-1 flex flex-col justify-between">
                          ...
                          <div className="space-y-4">
                            <div className="flex items-center justify-between gap-4 flex-wrap">
                              ...
                              <Badge
                                className={`text-[10px] font-bold uppercase tracking-widest px-3 py-1 border-2 rounded-full ${badgeTheme}`}
                              >
                                {exam.difficulty}
                              </Badge>
                            </div>
                          ...
                          {isCompleted && attempt ? (
                            <div className="w-full bg-emerald-50 border-2 border-emerald-200 p-4 rounded-xl flex items-center justify-between shadow-inner gap-4">
                              <div className="flex flex-col min-w-0">
                                ...
                                <span className="text-xs font-bold text-slate-800 mt-1 leading-snug">Saved to Valuation Core</span>
                              </div>
                            ...
                            attempt && (
                              <div className="w-full bg-sky-50 border-2 border-sky-200 p-4 rounded-xl flex items-center justify-between shadow-inner gap-4">
                                <div className="flex items-center gap-2 min-w-0">
                                  ...
                                  <p className="text-[10px] font-black text-sky-700 uppercase tracking-widest truncate">
                                    Rescue Checkpoint Active
                                  </p>
                                </div>
