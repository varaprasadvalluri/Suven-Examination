...
export const PerformanceStressTester: React.FC = () => {
  ...
  const [selectedScenario, setSelectedScenario] = useState<'exam-submission' | 'link-ingress' | 'proctor-heartbeat' | 'security-pin'>(
    'exam-submission'
  );
    ...
    if (isRunning) {
      interval = setInterval(() => {
        setElapsedTime((prev) => {
          ...
          const currentSimulatedUsers = Math.min(targetUsers, Math.round((nextTime / rampUpDuration) * targetUsers));
          ...
          const simulatedLatency = Math.round((actualPing || 110) * intensityMultiplier * userLoadStress + (Math.random() * 35 - 15));
  ...
  const startStressTest = () => {
    ...
    setTimeout(() => {
      setExploitLogs((prev) => [
        ...
        {
          msg: '⚠️ [DDoS THRESHOLD] Concurrent Firestore client rate estimated: 8,450 op/min (Safely below single doc limit)',
          type: 'warn'
        }
      ]);
    }, 4000);
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-1 shadow-xl border border-slate-100 rounded-[28px] overflow-hidden bg-white">
          <CardHeader className="p-6 border-b border-slate-100 bg-slate-50/50">
            ...
            <CardDescription className="text-xs font-semibold text-slate-400">
              Specify scaling parameters to stress test your database nodes.
            </CardDescription>
          </CardHeader>
        ...
        <div className="lg:col-span-2 space-y-6 flex flex-col justify-between">
          ...
          <div
            className={`p-4 rounded-3xl border-2 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-sm transition-all duration-300 ${
              cbState === 'OPEN'
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={chartData} margin={{ top: 10, right: -10, left: -20, bottom: 0 }}>
                      ...
                      <Line
                        yAxisId="right"
                        type="monotone"
                        name="Resilient Success Rate (%)"
                        dataKey="resilientSuccessRate"
                        stroke="#10B981"
                        strokeWidth={3}
                        dot={false}
                        activeDot={{ r: 6 }}
                      ...
                      <Line
                        yAxisId="right"
                        type="monotone"
                        name="Unprotected Success Rate (%)"
                        dataKey="unprotectedSuccessRate"
                        stroke="#EF4444"
                        strokeWidth={2.2}
                        strokeDasharray="3 3"
                        dot={false}
        ...
        <CardHeader className="p-6 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-base font-black text-slate-900 uppercase tracking-tight">
              Heavy Load Architectural Recommendation & Mitigation Matrix
            </CardTitle>
          </div>
          <CardDescription className="text-xs font-semibold text-slate-400">
            Identify potential scaling limitations and proactive Firestore/system design improvements.
          </CardDescription>
        </CardHeader>
