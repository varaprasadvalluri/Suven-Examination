...
export const AdminCloudBilling: React.FC = () => {
  ...
  const fetchRealGcpMetrics = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(
          `Connected to GCP Account: ${liveGcpData.userAccount || 'suveen2619@gmail.com'} (${liveGcpData.targetProjectId || 'project-02bb6275-51ac-45e7-940'})`
        );
      }
    } catch (e) {
  ...
  const totalFirestoreCost = parseFloat((firestoreReadCost + firestoreWriteCost + firestoreStorageCost + 4.8).toFixed(2));

  const cloudRunCost = 18.4;
  const storageEgressCost = 3.6;
  const cloudSqlCost = 9.2;
  ...
  const dailyCostTrend = [
    { day: 'Jul 15', Firestore: 0.45, CloudRun: 0.6, CloudSQL: 0.3, Other: 0.15 },
    ...
    { day: 'Jul 19', Firestore: 0.65, CloudRun: 0.7, CloudSQL: 0.38, Other: 0.18 },
  ...
  const handleUpdateBudget = () => {
    ...
    toast.success(`Monthly GCP Budget updated to $${parsedBudget.toFixed(2)}`);
  };

  const handleExportCsv = () => {
    let csv = 'GCP Service,Category,Current Cost ($),Projected Month End ($),Unit Rate,Usage Metric\n';
    servicesData.forEach((service) => {
      csv += `"${service.name}","${service.category}",${service.currentCost},${service.projectedCost},"${service.unitRate}","${service.usageMetric}"\n`;
    });
    ...
    downloadLink.download = `SuvenEdu_GCP_Billing_Report_${new Date().toISOString().split('T')[0]}.csv`;
    downloadLink.click();
    ...
    <div className="space-y-8 font-sans pb-12">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2 max-w-2xl">
            ...
            <p className="text-xs text-slate-300 leading-relaxed">
              Live real-time resource telemetry, cost allocation, and Cloud Billing integration for your Google Cloud account{' '}
              <span className="font-bold text-white">{gcpApiData?.userAccount || 'suveen2619@gmail.com'}</span> (Project:{' '}
              <span className="font-mono text-indigo-300">{gcpApiData?.targetProjectId || 'project-02bb6275-51ac-45e7-940'}</span>).
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 shrink-0">
            <a
              href={
                ...
                'https://console.cloud.google.com/welcome/new?authuser=1&project=project-02bb6275-51ac-45e7-940'
              }
              target="_blank"
              rel="noreferrer"
            >
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-300 font-mono pt-1">
              <span>
                Project ID: <strong className="text-white">{gcpApiData?.targetProjectId || 'project-02bb6275-51ac-45e7-940'}</strong>
              </span>
              <span>•</span>
              <span>
                Project Number: <strong className="text-white">{gcpApiData?.projectNumber || '489976275182'}</strong>
              </span>
              <span>•</span>
              <span>
                Billing Account:{' '}
                <strong className="text-emerald-300">
                  {gcpApiData?.billingInfo?.billingAccountName || 'billingAccounts/01B5E8-SUVEN-EDU-01'}
                </strong>
              </span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <Card className="bg-white border-slate-200/80 shadow-sm rounded-2xl relative overflow-hidden">
          <CardContent className="p-5 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Current Cycle Spend</span>
            ...
            <div>
              <div className="text-2xl md:text-3xl font-serif font-black text-slate-900">${currentTotalSpend.toFixed(2)}</div>
            ...
            <div className="space-y-1 pt-1">
              <div className="flex justify-between text-[10px] font-bold">
                ...
                <span className={budgetUsedPercentage > 85 ? 'text-rose-600' : 'text-emerald-600'}>{budgetUsedPercentage}%</span>
              </div>
              <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                <div
                  className={`h-full transition-all duration-500 ${
                    budgetUsedPercentage > 85 ? 'bg-rose-500' : budgetUsedPercentage > 60 ? 'bg-amber-500' : 'bg-emerald-500'
                  }`}
        ...
        <Card className="bg-white border-slate-200/80 shadow-sm rounded-2xl">
          <CardContent className="p-5 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Firestore DB Cost</span>
            ...
            <div>
              <div className="text-2xl md:text-3xl font-serif font-black text-slate-900">${totalFirestoreCost.toFixed(2)}</div>
        ...
        <Card className="bg-white border-slate-200/80 shadow-sm rounded-2xl">
          <CardContent className="p-5 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Cloud Run Compute</span>
            ...
            <div>
              <div className="text-2xl md:text-3xl font-serif font-black text-slate-900">${cloudRunCost.toFixed(2)}</div>
              <p className="text-[10px] font-medium text-slate-400 mt-0.5">Port 3000 Ingress • 0.5 vCPU / 512MB RAM</p>
            </div>
        ...
        <Card className="bg-white border-slate-200/80 shadow-sm rounded-2xl">
          <CardContent className="p-5 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Cloud SQL Database</span>
              <div className="w-8 h-8 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center font-bold">
                <Server size={18} />
              </div>
            </div>
            ...
            <div>
              <div className="text-2xl md:text-3xl font-serif font-black text-slate-900">${cloudSqlCost.toFixed(2)}</div>
              <p className="text-[10px] font-medium text-slate-400 mt-0.5">Developer PostgreSQL Instance</p>
            </div>
          ...
          <Card className="bg-white border-slate-200/80 shadow-sm rounded-3xl p-6">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">Granular unit rates, operating parameters, and month-end cost projections.</p>
              </div>
            </div>
            ...
            <div className="divide-y divide-slate-100 mt-2">
              {servicesData.map((svc) => {
                ...
                return (
                  <div
                    key={svc.id}
                    className="py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-50/60 p-3 rounded-2xl transition-all"
                  >
                    ...
                    <div className="text-left sm:text-right shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100">
                      <div className="text-base font-serif font-black text-slate-900">${svc.currentCost.toFixed(2)}</div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <Card className="bg-white border-slate-200/80 shadow-sm rounded-3xl p-6 space-y-5">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              ...
              <Badge className="bg-indigo-50 text-indigo-700 border-indigo-200 text-[10px]">Active Safeguard</Badge>
            </div>

            {!isSettingBudget ? (
              <div className="space-y-4">
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-2">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 block">Monthly Budget Cap</span>
                  <div className="flex items-center justify-between">
                ...
                <div className="space-y-2 text-xs font-medium">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">Alert Triggers</span>
            ...
            <div className="space-y-3 max-h-[380px] overflow-y-auto pr-1">
              {activityLogs.map((log) => (
                <div key={log.id} className="p-3 bg-slate-50 border border-slate-100 rounded-2xl space-y-1.5 text-xs">
                  ...
                  <p className="text-slate-800 font-medium leading-tight text-[11px]">{log.message}</p>
