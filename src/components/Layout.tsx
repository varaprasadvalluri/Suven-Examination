...
const CuteBillingIcon = () => (
  ...
  >
    ...
    <path
      d="M 12 10.2 V 13.8 M 10.8 11.2 C 10.8 11.2 11.2 10.8 12 10.8 C 12.8 10.8 13.2 11.2 13.2 11.8 C 13.2 12.8 10.8 12.2 10.8 13.2 C 10.8 13.8 11.4 14.2 12 14.2 C 12.8 14.2 13.2 13.8 13.2 13.8"
      stroke="#111222"
      strokeWidth="1.2"
      strokeLinecap="round"
    />
...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const toggleSchoolContext = async () => {
    ...
    let nextSchoolId: string | null = null;
    if (isGlobal) {
      // Scope to a real school rather than a fabricated placeholder ID — pick the first one
      // that actually exists. An admin with no schools onboarded yet has nothing real to
      // scope to, so surface that instead of silently attaching to a made-up school.
      try {
        const schools = await schoolsService.fetchAll();
        if (schools.length === 0) {
          toast.error('No schools exist yet — onboard a school before switching to Specific School scope.');
          return;
        }
        nextSchoolId = schools[0].id;
      } catch (err) {
        console.error(err);
        toast.error('Failed to look up a school to scope to.');
        return;
      }
    }
    try {
      await usersService.updateProfile(profile.uid, { schoolId: nextSchoolId });
    ...
    >
      ...
      <aside
        className={`fixed inset-y-0 lg:relative lg:translate-x-0! w-[280px] bg-white text-slate-800 flex flex-col shrink-0 z-50 transition-transform duration-300 ease-out ${
          ...
          isSidebarOpen ? 'p-5 shadow-[-4px_8px_30px_rgba(163,191,220,0.15)] bg-white' : 'p-0 overflow-hidden border-none! shadow-none!'
        } ${
          isStudent
      ...
      >
        ...
        <nav className="flex-1 px-1 py-6 space-y-4 overflow-y-auto">
          ...
          {filteredMenuItems.map((item) => {
            ...
            const colors = themeColors[item.label] || {
              gradient: 'from-sky-100 to-sky-50',
              activeBorder: 'border-sky-400',
            ...
            return (
              ...
              >
                ...
                <span
                  className={`text-[14px] tracking-wide truncate ${active ? 'font-black font-display text-indigo-950' : 'font-semibold'}`}
                >
                  {item.label}
                </span>

                {active && <ChevronRight className="ml-auto h-5 w-5 stroke-[3px] text-indigo-950" />}
              </Link>
            );
          })}
        </nav>
      ...
      {/* Main Content Area */}
      <main className={`flex-1 flex flex-col min-w-0 overflow-hidden relative z-10 transition-all ${isStudent ? 'lg:order-first' : ''}`}>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-3">
              <div className="text-right hidden sm:block">
                <p className="text-xs font-black text-indigo-950 leading-none tracking-wide font-display">
              ...
              <Avatar className="h-11 w-11 border-2 border-slate-200 shadow rounded-xl bg-slate-50 p-0.5">
                ...
                <AvatarFallback className="bg-indigo-600 text-white font-black text-xs font-display">
