...
export const ResultDetails: React.FC = () => {
  ...
  // Live subscription (not a one-shot fetch) on the attempt doc: exam submission now grades
  // asynchronously via Cloud Tasks (server/lib/taskQueue.ts) — the attempt lands here as
  // status='submitted' first, then flips to 'completed' (or 'grading_failed') once the
  // worker finishes. onSnapshot means this screen updates the instant that happens, no
  // polling or manual refresh needed — same pattern ExamInterface.tsx already uses for its
  // own live attempt/exam subscriptions. Exam + questions don't change after an attempt is
  // created, so those stay one-shot fetches, done once the first time the attempt loads.
  useEffect(() => {
    if (!attemptId) return;
    let examAndQuestionsLoaded = false;

    const unsubAttempt = onSnapshot(
      ...
      async (attemptSnap) => {
        ...
        if (!examAndQuestionsLoaded) {
          examAndQuestionsLoaded = true;
          try {
        ...
        <Card className="w-full max-w-lg border-rose-500 shadow-2xl rounded-[32px] overflow-hidden bg-white border-t-8 border-t-rose-600">
          <div className="p-10 text-center space-y-6">
            ...
            <CardDescription className="text-slate-500 text-xs font-semibold leading-relaxed">
              Examination Registry boundaries prevent candidates and proctors of other schools from intersecting raw diagnostics performance
              parameters.
            </CardDescription>
            <Button
              variant="default"
  ...
  // Grading runs asynchronously now (server/lib/taskQueue.ts) — the attempt can land here
  // already submitted but not yet graded. The onSnapshot subscription above means this
  // screen swaps to the full result view on its own the instant status flips to 'completed',
  // no refresh needed.
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-5 px-4 text-center">
        ...
        <div className="space-y-1.5">
          <p className="text-slate-800 font-bold">Grading your exam...</p>
          <p className="text-slate-500 text-xs font-medium max-w-xs">
            Your answers are saved. This page will update automatically the moment your result is ready.
          </p>
        </div>
      </div>
    );
  ...
  if (attempt.status === 'grading_failed') {
    return (
      <div className="min-h-[60vh] flex items-center justify-center p-4">
        <Card className="w-full max-w-lg border-amber-500 shadow-2xl rounded-[32px] overflow-hidden bg-white border-t-8 border-t-amber-500">
          <div className="p-10 text-center space-y-6">
            <div className="h-16 w-16 bg-amber-50 border-2 border-amber-200 text-amber-600 rounded-full flex items-center justify-center mx-auto">
              <ShieldAlert size={32} />
            </div>
            <CardTitle className="text-xl font-black text-slate-900 uppercase tracking-tight">Grading Delayed</CardTitle>
            <CardDescription className="text-slate-500 text-xs font-semibold leading-relaxed">
              Your answers were saved successfully, but grading is taking longer than expected. Please contact your school — your submission
              is not lost.
            </CardDescription>
          </div>
        </Card>
    ...
    return (
      <div className="max-w-3xl mx-auto space-y-8 pb-20">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <Button variant="outline" size="sm" className="rounded-xl font-bold text-xs uppercase" onClick={() => window.print()}>
            Print Receipt
          </Button>
        </div>
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="space-y-4 flex-1">
              ...
              <div className="pt-2 text-xs text-slate-500 leading-normal">
                ...
                <p>
                  <strong>Timestamp:</strong> {(attempt.endTime || attempt.startTime || '').slice(0, 19).replace('T', ' ')} UTC
                </p>
              </div>
            </div>
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-20">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <Button
          ...
          onClick={() => navigate(profile?.role === 'student' ? '/student/dashboard' : '/')}
        >
        ...
        <div className="flex items-center gap-3">
          ...
          {profile?.role === 'student' && (
            <Button
              variant="outline"
              size="sm"
              className="rounded-xl font-bold text-xs uppercase border-rose-200 text-rose-600 hover:bg-rose-50"
              onClick={handleLogout}
            >
      ...
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="bg-[#0F172A] p-6 md:p-10 rounded-[32px] text-white shadow-2xl relative overflow-hidden"
      >
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12">
          <div className="space-y-4 text-center md:text-left flex-1">
            ...
            <h1 className="text-3xl md:text-5xl font-display font-black tracking-tighter leading-tight">{exam.title}</h1>
            <div className="flex flex-wrap gap-4 items-center justify-center md:justify-start">
              ...
              <div className="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-xl border border-white/5">
                ...
                <span className="text-xs font-mono font-medium text-slate-300">
                  Marks: {attempt.score} / {exam.totalMarks}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="relative">
            <div className="w-48 h-48 rounded-full border-8 border-white/5 flex items-center justify-center relative">
              <svg className="absolute inset-0 w-full h-full -rotate-90">
                <circle cx="96" cy="96" r="88" fill="transparent" stroke="currentColor" strokeWidth="8" className="text-white/10" />
                <circle
                  cx="96"
                  cy="96"
                  r="88"
      ...
      {analytics && (
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card
            className={`shadow-xl shadow-slate-200/50 border-0 rounded-[32px] overflow-hidden group border ${analytics.violations > 0 ? 'bg-red-50 border-red-100' : 'bg-emerald-50 border-emerald-100'}`}
          >
            ...
            <CardContent className="space-y-6">
              <div className="flex flex-col items-center justify-center py-4">
                <div
                  className={`p-6 rounded-full mb-4 ${analytics.violations > 0 ? 'bg-white text-red-600 shadow-xl shadow-red-200' : 'bg-white text-emerald-600 shadow-xl shadow-emerald-200 animate-pulse'}`}
                >
        ...
        <CardHeader className="p-5 md:p-10 pb-0">
          <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
            <div className="flex items-center gap-3">
              ...
              <CardTitle className="text-xl md:text-2xl font-display font-bold text-slate-900 group-hover:text-indigo-600 transition-colors uppercase tracking-tight">
                Time Persistence Analysis
              </CardTitle>
            </div>
          ...
          <CardDescription className="text-slate-500 font-medium md:ml-13">
            Comparative per-item response latency vs. Institutional Topper (Top 1%).
          </CardDescription>
        </CardHeader>
        <CardContent className="p-4 md:p-10 h-[320px] md:h-[400px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={timeComparisonData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              ...
              <Bar name="My Time" dataKey="student" fill="#4f46e5" radius={[6, 6, 0, 0]} barSize={24} />
              <Bar name="Topper Avg" dataKey="topper" fill="#e2e8f0" radius={[6, 6, 0, 0]} barSize={24} />
            </BarChart>
          </ResponsiveContainer>
        </CardContent>
        ...
        <div className="grid grid-cols-1 gap-6">
          {questions.map((q, idx) => {
            ...
            return (
              ...
              >
                <Card
                  className={`group shadow-lg hover:shadow-2xl transition-all duration-300 border-0 rounded-[24px] overflow-hidden ${isCorrect ? 'shadow-emerald-100' : 'shadow-red-100'}`}
                >
                      ...
                      <div className="p-5.5 bg-slate-50 border border-slate-200 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4">
                        <div className="flex flex-col">
                          ...
                          {q.type === 'math' && studentAnswer !== null && studentAnswer !== undefined ? (
                            ...
                            <span className="font-mono text-xl font-bold mt-1 text-slate-800">
                              {studentAnswer !== null && studentAnswer !== undefined ? String(studentAnswer) : 'No Response'}
                            </span>
                          )}
                        </div>
                        <div className="flex flex-col">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {(q.options || []).map((opt, i) => {
                          ...
                          if (isCorrectOpt)
                            ...
                            variant = 'bg-red-50 border-red-500 text-red-900 font-bold shadow-sm shadow-red-100';

                          return (
                            <div
                              key={i}
                              className={`p-4 rounded-xl border-2 flex items-center justify-between transition-all group/opt ${variant}`}
                            >
                              <span className="flex items-center gap-3">
                                <span
                                  className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-black transition-colors ${isCorrectOpt ? 'bg-emerald-500 text-white' : isSelected ? 'bg-red-500 text-white' : 'bg-slate-50 text-slate-400 group-hover/opt:bg-indigo-50 group-hover/opt:text-indigo-400'}`}
                                >
