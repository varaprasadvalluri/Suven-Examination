...
export const ErrorBook: React.FC = () => {
  ...
  useEffect(() => {
    const fetchErrorBook = async () => {
      ...
      setLoading(true);
      try {
        const q = query(collection(db, 'error_books'), where('studentId', '==', user.uid), orderBy('createdAt', 'desc'));
        const snapshot = await getDocs(q);
        const errorBookEntries = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }) as ErrorBookEntry);
        setEntries(errorBookEntries);
      } catch (error) {
        console.error(error);
      } finally {
        setLoading(false);
          ...
          {subjects.map((s) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all cursor-pointer whitespace-nowrap ${
                selectedSubject === s ? 'bg-indigo-650 text-white shadow-md' : 'text-slate-400 hover:text-white'
              }`}
            >
              {s}
            </button>
            ...
            >
              <Card className="border border-[#20254C] shadow-2xl rounded-[32px] overflow-hidden bg-gradient-to-br from-[#13162C] to-[#121326] group hover:border-[#2C3362] transition-colors w-full min-w-[280px]">
                <div className="flex flex-col md:flex-row min-h-[220px]">
                  ...
                  <div className="md:w-64 bg-[#101224] p-5 md:p-8 flex flex-col justify-between shrink-0 border-b md:border-b-0 md:border-r border-[#1C2042]">
                    <div>
                  ...
                  <div className="flex-1 p-5 md:p-8 space-y-6 flex flex-col justify-center">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="p-4 rounded-xl bg-rose-500/5 border border-rose-500/15">
                        ...
                        <p className="text-sm font-bold text-slate-300 uppercase">
                          Option {String.fromCharCode(65 + entry.selectedAnswer)}
                        </p>
                      </div>
                    ...
                    <div className="pt-6 border-t border-[#1C2042] flex items-center justify-between flex-wrap gap-4">
                      ...
                      <Button
                        variant="ghost"
                        className="h-10 px-5 rounded-xl font-black text-[10px] uppercase tracking-widest text-[#B5F2D2] hover:text-white hover:bg-emerald-500/5 flex items-center gap-2 border border-transparent hover:border-emerald-500/15"
                      >
        ...
        {[
          {
            ...
            desc: 'AI-driven analysis of why mistakes happen.',
            color: 'bg-indigo-600/10 border-indigo-500/20 text-indigo-300'
          },
          {
            icon: PieChart,
            label: 'Trend Analysis',
            desc: 'Repeat mistakes automatically flagged for high priority.',
            color: 'bg-[#FF5A6A]/10 border-[#FF5A6A]/20 text-[#FF5A6A]'
          },
          {
            icon: RefreshCcw,
            label: 'Adaptive Cycles',
            desc: 'Mistakes re-injected dynamically into mock practice runs.',
          ...
          return (
            <div
              key={i}
              className="p-5 md:p-8 bg-gradient-to-br from-[#13162C] to-[#121326] border border-[#20254C] rounded-[24px] relative overflow-hidden group"
            >
