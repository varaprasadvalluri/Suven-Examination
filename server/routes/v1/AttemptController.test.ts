...
//
// This is the shape the DAO interfaces were designed to make possible (server/dao/*.ts): the
...
// identity the test set up. The auth logic itself has its own tests (server/authorization.test.ts).
...
vi.mock('../../lib/taskQueue', () => ({ taskQueueService: { enqueueGradingTask: mockEnqueueGradingTask } }));
vi.mock('../../authorization', async (importOriginal) => {
  const actual = (await importOriginal()) as any;
  return { ...actual, authorizeWrite: mockAuthorizeWrite };
});
