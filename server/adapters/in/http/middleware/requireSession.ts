import { verifySessionToken } from '../../../../application/services/tokens';
import { documentStore } from '../../../../composition';
import { logger } from '../../../../lib/logger';
import { RequestAuth } from '../../../../application/ports/RequestAuth';

// Re-exported so HTTP-layer code can keep importing the caller identity from the middleware
...
async function getActiveSessionId(uid: string): Promise<string | undefined> {
  ...
  const user = await documentStore.getById('users', uid);
  const activeSessionId = user.exists ? ((user.data as any)?.activeSessionId as string | undefined) : undefined;
