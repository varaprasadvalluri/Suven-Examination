...
import { GradingDispatcher } from '../ports/GradingDispatcher';
...
export class AttemptSubmissionService {
  constructor(
    ...
    private readonly grading: GradingDispatcher,
   ...
   * (ScoreVerificationService). Answers themselves ARE trusted: a student legitimately
   ...
   * isn't configured (local dev), the GradingDispatcher adapter falls back to grading
  ...
  async submit(auth: RequestAuth, attemptId: string, writeType: 'update' | 'set', data: any): Promise<SubmissionResult | SubmissionDenied> {
    ...
    const existing = await this.attempts.findById(attemptId);
    ...
    const decision = await this.authorization.authorizeWrite(auth, writeType, 'attempts', attemptId, submittedData);
    if (decision.ok === false) {
      return { ok: false, status: decision.status, error: decision.error };
    }

    const authorizedData = decision.data;
    await this.documents.write({ type: writeType, collectionName: 'attempts', docId: attemptId, data: authorizedData });
