...
export function scopeFieldFor(collectionName: string, role: ProxyRole): string | undefined {
  const scope = SCOPE_FIELD[collectionName];
  if (!scope) return undefined;
  return role === 'school' ? scope.school : role === 'student' ? scope.student : undefined;
}

export function scopeValueFor(auth: RequestAuth, role: ProxyRole): string | null {
...
export function injectReadScope(auth: RequestAuth, collectionName: string, constraints: any[]): any[] | null {
