...
// firebase-admin is only reached by verifyFirebaseIdToken, which needs real Google certs;
// stubbed so importing this module doesn't try to initialize an admin app.
vi.mock('firebase-admin/app', () => ({ initializeApp: vi.fn(() => ({ name: 'stub' })), getApps: vi.fn(() => []) }));
